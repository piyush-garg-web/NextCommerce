...
const OrderDetails = async ({params}) => {
  try {
        ...
        <div className="no-print">
          ...
          <div className="lg:px-32 px-5 my-20">
            <div>
              ...
              <div className="mb-5">
                <p><b>Order Id : </b>{plainOrderData?.order_id}</p>
                <p><b>Transaction Id : </b>{plainOrderData?.payment_id}</p>
                <p className="capitalize"><b>Status : </b>{plainOrderData?.status}</p>
              </div>
              <table className="w-full border">
                ...
                <tbody>
                  {plainOrderData && plainOrderData?.product?.map((product) => (
                    <tr key={product.variantId._id} className="md:table-row block border-b">
                      <td className="p-3">
                        <div className="flex items-center gap-5">
                          <Image src={product?.variantId?.media[0]?.secure_url || imgPlaceholder.src}
                            width={60}
                    ...
                    <table className="w-full">
                      <tbody>
                        <tr>
                          ...
                          <td className="text-end py-2">{plainOrderData?.name}</td>
                        </tr>
                        <tr>
                          <td className="font-medium py-2">Email</td>
                          <td className="text-end py-2">{plainOrderData?.email}</td>
                        </tr>
                        <tr>
                          <td className="font-medium py-2">Phone</td>
                          <td className="text-end py-2">{plainOrderData?.phone}</td>
                        </tr>
                        <tr>
                          <td className="font-medium py-2">Country</td>
                          <td className="text-end py-2">{plainOrderData?.country}</td>
                        </tr>
                        <tr>
                          <td className="font-medium py-2">State</td>
                          <td className="text-end py-2">{plainOrderData?.state}</td>
                        </tr>
                        <tr>
                          <td className="font-medium py-2">City</td>
                          <td className="text-end py-2">{plainOrderData?.city}</td>
                        </tr>
                        <tr>
                          <td className="font-medium py-2">Pincode</td>
                          <td className="text-end py-2">{plainOrderData?.pincode}</td>
                        </tr>
                        <tr>
                          <td className="font-medium py-2">Landmark</td>
                          <td className="text-end py-2">{plainOrderData?.landmark}</td>
                        </tr>
                        <tr>
                          <td className="font-medium py-2">Ordernote</td>
                          <td className="text-end py-2">{plainOrderData?.ordernote || '---'}</td>
                        </tr>
                      </tbody>
    ...
    return (
      <div className="flex justify-center items-center py-32">
        <h4 className="text-red-500 text-xl font-semibold">Something went wrong</h4>
      </div>
    )
  }
}
