...
<div>
  <button type='button' className="text-red-500 underline underline-offset-1 mb-2 cursor-pointer" onClick={()=>dispatch(removeFromCart({productId:product.productId,
...
<div className="h-32 border-t pt-5 px-2">
  <h2 className="flex justify-between items-center text-lg font-semibold"><span>Subtotal</span><span>{subtotal?.toLocaleString('en-IN',{style:'currency',currency:'INR'})}</span></h2>
   ...
   <div className="flex justify-between gap-5 mt-3">
    <Button type='button' asChild variant='secondary' className='w-[160px]' onClick={()=>setOpen(false)}>
      <Link href={WEBSITE_CART}>View Cart</Link>
    </Button>
    <Button type='button' asChild className='w-[160px]' onClick={()=>setOpen(false)}>
    {cart.count ?
